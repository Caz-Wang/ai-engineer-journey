...
def test_get_posts_with_mock_client():
    client = Mock()
    response = Mock()
    ...
    api_client.get_posts(1, client)
    client.get.assert_called_once_with(
    f"{api_client.API_BASE_URL}/posts",
    timeout=api_client.REQUEST_TIMEOUT,
)

def test_get_posts_handles_http_error():
    client = Mock()
    

    client.get.side_effect = requests.RequestException("HTTP error")
    ...
    print(client.get.call_args_list)

def test_get_posts_handles_http_error(monkeypatch):
    client = Mock()
    client.get.side_effect = requests.RequestException("HTTP error")
    sleep = Mock()
    ...
    call("https://jsonplaceholder.typicode.com/posts", timeout=10),
    call("https://jsonplaceholder.typicode.com/posts", timeout=10),
    call("https://jsonplaceholder.typicode.com/posts", timeout=10),
    ]

def test_get_posts_with_fake_client():
    client = FakeClient()
    posts = api_client.get_posts(1, client)

    assert len(posts) == 1
    assert posts[0].title == "Test post"
    assert client.url == "https://jsonplaceholder.typicode.com/posts"
